...
export const processPayuPayment = asyncHandler(async (req, res) => {
  ...
  const paymentData = {
    ...
    email,
    phone,
    surl: process.env.BACKEND_URL +`/api/v1/payment/payu/success`,
     furl: process.env.BACKEND_URL+`/api/v1/payment/payu/failure`,
    hash,
  };
...
export const payuSuccess = asyncHandler(async (req, res) => {
  ...
  if (status === "success") {
    const order = await Order.findOneAndUpdate(
      { "paymentInfo.id": txnid },
      { "paymentInfo.status": status, orderStatus: "Processing" },
      { new: true }
    );
    if (order) {
...
export const payuFailure = asyncHandler(async (req, res) => {
  ...
  await Order.findOneAndUpdate(
    { "paymentInfo.id": txnid },
    { "paymentInfo.status": status, orderStatus: "Payment Failed" }
  );
